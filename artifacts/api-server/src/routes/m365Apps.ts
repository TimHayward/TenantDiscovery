...
router.get("/m365/apps", async (req, res) => {
  try {
    const result = await getCached("m365-apps", async () => {
      ...
      // Build permission ID → scope name map from MS Graph SP
      ...
      // Authorization policy — who can register apps
      ...
      const apps = (appsResult.items as any[]).map((app) => {
        ...
        // Permissions from requiredResourceAccess
        ...
        // Flag apps using only secrets (no certs) as an architecture risk
        const secretOnly =
